...
independent_vars = [
    ...
    'SI_mean', 'EVI_mean', 'SAVI_mean', 'NDVI_mean', 'BSI_mean', 'CAI_mean',
    'BI_stdDev', 'CI_stdDev', 'NDMI_stdDev', 'OMI_stdDev', 'RI_stdDev',
    'SI_stdDev', 'EVI_stdDev', 'SAVI_stdDev', 'NDVI_stdDev', 'BSI_stdDev', 'CAI_stdDev'
]
...
np.random.seed(42)
test_fields = np.random.choice(unique_fields, size=int(0.2 * len(unique_fields)), replace=False)
...
for img_id, df_img in train_data.groupby("Image_id"):
    ...
    scores = cross_val_score(lasso_model, X_img_scaled, y_img, cv=kf, scoring=rmse_scorer)
    rmse_cv_mean = np.mean(scores)

    image_rmse_records.append({'Image_id': img_id, 'RMSE': rmse_cv_mean})

ranked_images_train = pd.DataFrame(image_rmse_records).sort_values(by='RMSE').reset_index(drop=True)
...
for i in range(1, len(ranked_images_train) + 1):
    ...
    X_train_scenario_scaled_selected = pd.DataFrame(X_train_scenario_scaled, columns=independent_vars)[important_features].values

    rf_model = RandomForestRegressor(random_state=42)
    kfold = KFold(n_splits=5, shuffle=True, random_state=42)
    cv_scores = cross_val_score(rf_model, X_train_scenario_scaled_selected, y_train_scenario,
                                cv=kfold, scoring='neg_mean_squared_error')
    cv_rmse_scores = np.sqrt(-cv_scores)

    rf_model.fit(X_train_scenario_scaled_selected, y_train_scenario)
    y_pred_scenario = rf_model.predict(X_test_scaled[:, [independent_vars.index(f) for f in important_features]])
...
sns.lineplot(data=results_df, x='Images_Used', y='RMSE', marker='o', label='RMSE (Test)', color='blue', ax=ax1)
sns.lineplot(data=results_df, x='Images_Used', y='MAE', marker='s', label='MAE (Test)', color='green', ax=ax1)
ax1.set_xlabel("Number of Images Used")
ax1.set_ylabel("RMSE & MAE")
ax1.legend(loc="upper left")

ax2 = ax1.twinx()
sns.lineplot(data=results_df, x='Images_Used', y='R2', marker='^', label='R² (Test)', color='red', ax=ax2)
...
def auto_threshold_selection(r2_track, max_threshold=0.05, step=0.001):
    ...
    for thresh in thresholds:
        ...
        drop_indices = [i-1 for i in range(2, len(r2_copy)) if r2_copy[i] < r2_copy[i-2] - thresh]
...
for i in range(1, len(ranked_images_train) + 1):
    ...
    X_test_scaled_selected = pd.DataFrame(X_test_scaled, columns=independent_vars)[important_features].values
    rf_model = RandomForestRegressor(random_state=42)
    rf_model.fit(X_train_scenario_scaled_selected, y_train_scenario)
...
bad_iterations = [i-1 for i in range(2, len(r2_track)) if r2_track[i] < r2_track[i-2] - r2_drop_threshold]
bad_images = ranked_images_train.iloc[bad_iterations]['Image_id'].tolist()
ranked_images_train = ranked_images_train[~ranked_images_train['Image_id'].isin(bad_images)].reset_index(drop=True)
...
for i in range(1, len(ranked_images_train) + 1):
    ...
    X_test_scenario_scaled_selected = pd.DataFrame(X_test_scenario_scaled, columns=independent_vars)[important_features].values
...
results_df['R2_savgol'] = savgol_filter(results_df['R2'], window_length=5 if len(results_df) >= 5 else len(results_df) - (len(results_df) % 2 == 0), polyorder=2)

# 📊 Final Plot
fig, ax1 = plt.subplots(figsize=(12, 6))
sns.lineplot(data=results_df, x='Images_Used', y='RMSE', marker='o', label='RMSE (Test)', color='blue', ax=ax1)
sns.lineplot(data=results_df, x='Images_Used', y='MAE', marker='s', label='MAE (Test)', color='orange', ax=ax1)
ax1.set_xlabel("Number of Images Used")
ax1.set_ylabel("RMSE & MAE")
ax1.legend(loc="upper left")

ax2 = ax1.twinx()
sns.lineplot(data=results_df, x='Images_Used', y='R2_smooth', marker='^', label='R² (Rolling)', color='red', ax=ax2)
sns.lineplot(data=results_df, x='Images_Used', y='R2_ewm', marker='x', label='R² (EWMA)', color='green', ax=ax2)
sns.lineplot(data=results_df, x='Images_Used', y='R2_savgol', marker='D', label='R² (Savitzky-Golay)', color='purple', ax=ax2)
ax2.set_ylabel("R² Score")
ax2.legend(loc="upper right")
...
data['soilTypes'] = data['soilTypes'].str.strip('[]').str.replace("'", "").str.split(', ')
...
ax2 = ax1.twinx()
sns.lineplot(data=results_df, x='Images_Used', y='R2_smooth', marker='^', label='R² (Smoothed)', color='red', ax=ax2)
ax2.set_ylabel("R² Score")
ax2.legend(loc="upper right")
...
# 1) Load
# =========================
data = pd.read_csv("/content/TrainingSession_Etezadi.csv").dropna().reset_index(drop=True)
...
print("Train fields:", train_df["Field_no"].nunique(), "Test fields:", test_df["Field_no"].nunique())
...
print("Inner-train fields:", len(np.unique(g_tr_in)), "Inner-val fields:", len(np.unique(g_val_in)))
...
if len(important_features) == 0:
    raise RuntimeError("Lasso selected 0 features. Consider lowering alpha grid / using ElasticNetCV / removing strict preprocessing.")
...
for img_id, df_img in train_inner.groupby("Image_id"):
    ...
    image_scores.append({"Image_id": img_id, "CV_RMSE": float(np.mean(cv_rmse)), "n": int(df_img.shape[0])})
...
ranked_good = ranked_images[~ranked_images["Image_id"].isin(bad_image_ids)].reset_index(drop=True)
...
X_train_final = scaler.transform(train_final[independent_vars].astype(float))[:, feat_idx]
...
sns.lineplot(data=val_good_df, x="Images_Used", y="RMSE_val", marker="o", label="RMSE (VAL)")
sns.lineplot(data=val_good_df, x="Images_Used", y="MAE_val", marker="s", label="MAE (VAL)")
...
plt.figure(figsize=(12, 6))
sns.lineplot(data=val_good_df, x="Images_Used", y="R2_smooth", marker="^", label="R² smooth (VAL)")
sns.lineplot(data=val_good_df, x="Images_Used", y="R2_ewm", marker="x", label="R² ewm (VAL)")
...
# تجمیع به سطح Field
field_pred = (pred_df
              .groupby('Field_no', as_index=False)
              .agg(
...
ax.set_title(
    "Field-level SOM Prediction with Soil Sampling Points",
    fontsize=14
)
ax.set_axis_off()
...
plt.title(f"Observed vs Predicted SOM (Samples vs Field Prediction)\nR² (approx): {r2:.3f}")
plt.grid(True)
plt.tight_layout()
plt.show()
...
g = g.merge(uncertainty[["Field_no", "som_std", "som_iqr"]],
            left_on="FIELD", right_on="Field_no", how="left")
...
print("pred_df head:\n", pred_df.head())
print("unique fields in pred_df:", pred_df["Field_no"].nunique() if "Field_no" in pred_df.columns else "NO Field_no")
print("unique images in pred_df:", pred_df["Image_id"].nunique() if "Image_id" in pred_df.columns else "NO Image_id")
...
if FIELD_KEY_COL not in fields_gdf.columns:
    raise ValueError(f"'{FIELD_KEY_COL}' not found in shapefile. Available: {fields_gdf.columns.tolist()}")

# Ensure join key is numeric
fields_gdf[FIELD_KEY_COL] = pd.to_numeric(fields_gdf[FIELD_KEY_COL], errors="coerce").astype("Int64")
...
if missing_pred_cols:
    raise ValueError(f"pred_df missing columns: {missing_pred_cols}. Available: {pred_df.columns.tolist()}")
...
print("pred_df unique fields:", tmp[PRED_DF_FIELD_COL].nunique(), "unique images:", tmp[PRED_DF_IMG_COL].nunique())
...
uncertainty = (
    tmp.groupby(PRED_DF_FIELD_COL, as_index=False)
       .agg(
...
field_level = (
    tmp.groupby(PRED_DF_FIELD_COL, as_index=False)
       .agg(
           som_pred=(PRED_DF_PRED_COL, "median"),
           n_images=(PRED_DF_IMG_COL, "nunique")
       )
)
...
if len(g_obs_pred) > 0:
    ...
    print("⚠️ Observed vs Predicted map skipped: som_obs is missing (no mean_Om_p in pred_df).")
...
if len(g_err_valid) > 0:
    ...
    ax.set_title(f"Predicted SOM with High-error Fields Highlighted (Top {int(TOP_FRAC*100)}%)")
    ax.set_axis_off()
    plt.tight_layout()
    plt.show()
    ...
    worst_fields = (
        g_err_valid[[FIELD_KEY_COL, "som_pred", "som_obs", "abs_error", "som_iqr", "som_std", "n_images"]]
    ...
    print("⚠️ High-error map skipped: som_obs is missing, so abs_error cannot be computed.")
